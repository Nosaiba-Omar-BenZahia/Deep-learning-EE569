...
N_FEATURES = 28 * 28
NUM_CLASSES = 10
LAYERS = [(128, "sigmoid"), (64, "sigmoid"), (NUM_CLASSES, "softmax")]
...
def build_mlp():
    ...
    for layer_size, activation_function in LAYERS:
        input_node = create_mlp_layer(input_node, output_dim, layer_size, activation_function)
...
X_train = X_train.reshape(num_train_samples, N_FEATURES).astype("float32") / 255.0
X_test = X_test.reshape(num_test_samples, N_FEATURES).astype("float32") / 255.0
...
for epoch in range(EPOCHS):
    ...
    print(f"Epoch {epoch + 1}, Loss: {avg_loss:.6f}, LR: {LEARNING_RATE:.4f}")

print(f"Training Time: {time() - start:.4f} seconds")
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title(f"Training Loss (MLP, batch={BATCH_SIZE})")
plt.show()
